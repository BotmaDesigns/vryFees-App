	  ...
	  return response.json();
		...
		if (!UpToDate)
		{
			...
			fetch('http://vryfees.botmasoftware.com/api.ashx', {
				...
				printData(JSON.parse(localStorage.getItem("data")))
			}).catch(function() {
				console.log("Booo2");
			});
		}
		...
		for (i=0;i<data.length;i++){
			for (j=0;j<data[i].Schedules.length;j++){
				if (data[i].Schedules[j].Venue.Name == venueNum){
					...
					document.getElementById("schedule").innerHTML += "<tr><td><a href='show.html?showNum=" + i + "'>" + data[i].Name + "</a></td><td>" + date.toDateString() + "</td><td>" + date.getHours() + ":" + (date.getMinutes()<10?'0':'') + date.getMinutes() + "</td></tr>";
				}
			}
		}
		}).catch(function() {
